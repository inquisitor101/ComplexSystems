...
for t=1:finalTime-1  % time 
    % monitor progress
    disp(['step: ', num2str(t), ' of ', num2str(finalTime)]);
    ...
    randomRotation = pi/2*gaussianDist([-1, +1], 0.01, N);
    ...
    Vx(idx2, t+1) = Vx(idx2, t) + sgn(idx2).*theta*dt;
    Vy(idx2, t+1) = Vy(idx2, t) + sgn(idx2).*theta*dt;
...
%     [bb, elong(t+1)] = boundingBox(Cx(:, t), Cy(:, t), h(t+1));
